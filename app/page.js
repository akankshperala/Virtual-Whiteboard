    ...
    <div className="w-screen h-screen relative overflow-hidden bg-amber-100">
      <div className="relative h-full w-full bg-black">
        ...
        <div className="absolute bottom-0 left-0 right-0 top-0 bg-[linear-gradient(to_right,#4f4f4f2e_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:14px_24px]">
          ...
          {/* Whiteboard */}
          <WhiteboardCanvas color={color} stroke={stroke} eraser={eraser}/>

          {/* Toolbar */}
          <ToolBar isMenuOpen={isMenuOpen} setColor={setColor} setStroke={setStroke} setiserasing={setiserasing} iserasing={iserasing} />
        </div>
      </div>
    </div>
  );
